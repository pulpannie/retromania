...
public class CellManager {
  ...
  /**
   * @param userPreference Current user's TicTacToe game's preference. Instantiates all the Cells in
   *     the TicTacToe game.
   */
   ...
   */
  void touchCell(int i, int j, String currentTurn) {
   ...
   */
  boolean isCellTouched(int i, int j) {
   ...
   */
  String getCellState(int i, int j) {
   ...
   * @return true if it is the case.
   */
  boolean checkDiagLeft() {
    for (int i = 0; i < userPreference.getGameSize() - 2; i++) {
      for (int j = 0; j < userPreference.getGameSize() - 2; j++) {
        if (cellArray[i][j].equal(cellArray[i + 1][j + 1])
            && cellArray[i + 1][j + 1].equal(cellArray[i + 2][j + 2])) {
          winner = cellArray[i][j].getCell();
          return true;
        }
      }
  ...
  /**
   * checks if there is are three consecutive Cells with the same state, diagonally going down
   * right.
   *
   * @return true if it is the case.
   */
  boolean checkDiagRight() {
    for (int i = 0; i < userPreference.getGameSize() - 2; i++) {
      for (int j = 0; j < userPreference.getGameSize() - 2; j++) {
        if (cellArray[j][userPreference.getGameSize() - i - 1].equal(
                cellArray[j + 1][userPreference.getGameSize() - i - 2])
            && (cellArray[j + 1][userPreference.getGameSize() - i - 2])
                .equal(cellArray[j + 2][userPreference.getGameSize() - i - 3])) {
