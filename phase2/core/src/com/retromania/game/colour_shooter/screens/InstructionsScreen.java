...
public class InstructionsScreen implements Screen {
    ...
    public InstructionsScreen(RetroManiaGame game, ColourShooterStarter mainscreen) {
        ...
        width = 400;
        height = 800;
        stage = new Stage(new StretchViewport(width,
    ...
    public void show() {
        ...
        Image instructionsImg = new Image(new Texture
                (Gdx.files.internal("colour_shooter/instructions.png")));
        instructionsImg.setSize((float) (width * 0.85), (float) (height * 0.75));
        instructionsImg.setPosition((float) (width/2.2) - (instructionsImg.getWidth()/2),
                (float) (height/2.2) - (instructionsImg.getHeight()/2));
        stage.addActor(instructionsImg);

        Image backImage = new Image(new Texture
                (Gdx.files.internal("colour_shooter/back_wth_txt.png")));
        backImage.setSize((float) (width * 0.4), (float) (height * 0.1));
        backImage.setPosition(width / 2 - (backImage.getWidth() / 2),
                (float) (height * 0.87));
